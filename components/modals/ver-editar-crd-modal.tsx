...
export function VerEditarCRDModal({ crd, onClose }: Props) {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      await crdsApi.update(crd.documentId, {
        estado: form.estado,
        ...
        <div className="overflow-y-auto flex-1 p-4 sm:p-6 flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {editing ? (
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, fechaCreacion: e.target.value })
                  }
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {editing ? (
                <select
                  value={form.estado}
                  onChange={(e) =>
                    setForm({ ...form, estado: e.target.value as CRDEstado })
                  }
                  className="px-3 py-2 border border-[var(--border)] rounded-lg text-sm focus:outline-none focus:border-[var(--brand-teal)] transition-colors"
                >
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {editing ? (
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      consentimientoFirmado: e.target.checked,
                    })
                  }
              ...
              <span
                className={`flex items-center gap-1.5 text-sm font-semibold ${form.consentimientoFirmado ? "text-green-600" : "text-red-500"}`}
              >
                {form.consentimientoFirmado ? (
            ...
            <Row
              ...
              value={
                <span className="text-lg font-bold text-[var(--foreground)]">
                  {form.administraciones?.length ?? 0}
                </span>
              }
            />
            <Row
