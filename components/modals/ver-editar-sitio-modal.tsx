...
export function VerEditarSitioModal({ sitio, onClose }: Props) {
  ...
  useEffect(() => {
    if (sitio) { setForm({ ...sitio }); setEditing(false); setSaved(false); setError(null) }
  }, [sitio])
  ...
  const handleSave = async () => {
    ...
    setError(null)
    try {
      await sitiosApi.update(sitio.id, {
        nombre: form.nombre,
        codigo: form.codigo,
      })
      setSaved(true)
  ...
  const Field = ({ label, fkey }: { label: string; fkey: keyof SitioClinico }) => (
        ...
        <div className="flex items-start justify-between p-4 sm:p-6 border-b border-[var(--border)]">
          ...
          <div className="flex items-center gap-2">
            ...
            {!editing ? (
              ...
              <>
                <button onClick={() => { setForm({ ...sitio }); setEditing(false); setError(null) }} className="px-3 py-1.5 text-sm text-[var(--muted-foreground)] border border-[var(--border)] rounded-lg hover:bg-[var(--muted)] transition-colors">Cancelar</button>
        ...
        <div className="overflow-y-auto flex-1 p-4 sm:p-6 flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 gap-4 p-4 bg-[var(--muted)] rounded-xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[11px] font-semibold uppercase text-[var(--muted-foreground)]">Pacientes vinculados</p>
                <p className="text-2xl font-bold text-[var(--foreground)]">{form.pacientes?.length ?? 0}</p>
              </div>
            </div>
          </div>
        </div>
