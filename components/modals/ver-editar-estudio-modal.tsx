...
export function VerEditarEstudioModal({ estudio, onClose }: Props) {
  ...
  useEffect(() => {
    if (estudio) { setForm({ ...estudio }); setEditing(false); setSaved(false); setError(null) }
  }, [estudio])
  ...
  const handleSave = async () => {
    ...
    setError(null)
    try {
      await estudiosApi.update(estudio.id, {
        ...
        <div className="flex items-start justify-between p-4 sm:p-6 border-b border-[var(--border)]">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0 ml-2">
            ...
            {!editing ? (
              ...
              <>
                <button onClick={() => { setForm({ ...estudio }); setEditing(false); setError(null) }} className="px-3 py-1.5 text-sm text-[var(--muted-foreground)] border border-[var(--border)] rounded-lg hover:bg-[var(--muted)] transition-colors">Cancelar</button>
        ...
        <div className="overflow-y-auto flex-1 p-4 sm:p-6 flex flex-col gap-5">
          ...
          <div className="grid grid-cols-3 gap-3 p-4 bg-[var(--muted)] rounded-xl">
            <div className="text-center">
              ...
              <p className="text-lg font-bold text-[var(--foreground)]">{form.pacientes?.length ?? 0}</p>
            ...
            <div className="text-center border-x border-[var(--border)]">
              ...
              <p className="text-lg font-bold text-[var(--foreground)]">{form.sitios_clinicos?.length ?? 0}</p>
              <p className="text-[11px] text-[var(--muted-foreground)] font-medium uppercase">Sitios Clínicos</p>
            </div>
            <div className="text-center">
              <Calendar className="w-4 h-4 text-blue-500 mx-auto mb-1" />
              <p className="text-sm font-semibold text-[var(--foreground)]">{form.fechaFinVigilancia}</p>
              <p className="text-[11px] text-[var(--muted-foreground)] font-medium uppercase">Fin Vigilancia</p>
            </div>
          </div>
        </div>
      </div>
