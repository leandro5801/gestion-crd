...
export function EventosAdversosContent() {
  ...
  if (search) {
    filters.$or = [
      { SOC: { $containsi: search } },
    ]
  }

  const { items: eventos, meta, isLoading, mutate } = useEventosAdversos({
  ...
  return (
    <>
      <NuevoEventoModal open={showModal} onClose={() => { setShowModal(false); mutate() }} />
      <VerEditarEventoModal evento={selected} onClose={() => { setSelected(null); mutate() }} />
        ...
        <div className="bg-white rounded-xl border border-[var(--border)] p-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                value={intensidad}
                onChange={(e) => { setIntensidad(e.target.value); setPage(1) }}
              ...
              >
                {['Todos', 'Severo', 'Moderado', 'Leve'].map((s) => <option key={s}>{s}</option>)}
              </select>
            </div>
            <button
        ...
        <div className="bg-white rounded-xl border border-[var(--border)]">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[var(--border)]">
                  {['SOC / Tipo', 'Gravedad', 'Intensidad', 'Imputabilidad', 'Suspensión', 'Tipo EA', 'Action'].map((h) => (
              ...
              <tbody>
                ...
                {!isLoading && eventos.map((e) => {
                  const tipoEA = typeof e.tipo_evento_adverso === 'object' ? e.tipo_evento_adverso : null
                  return (
                    <tr key={e.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--muted)] transition-colors">
                      <td className="px-5 py-4">
                        <p className="text-sm font-semibold text-[var(--foreground)]">{e.SOC ?? '—'}</p>
                      </td>
                      <td className="px-5 py-4 text-xs text-[var(--foreground)]">{e.gravedad}</td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4">
                        <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${e.suspensionTratamiento ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-green-50 text-green-700 border border-green-200'}`}>
          ...
          <div className="px-5 py-4 flex items-center justify-between border-t border-[var(--border)]">
            <p className="text-sm text-[var(--muted-foreground)]">
              Showing <span className="font-semibold text-[var(--foreground)]">{eventos.length}</span> of{' '}
              <span className="font-semibold text-[var(--foreground)]">{totalEventos.toLocaleString()}</span> entries
            </p>
          ...
          <div className="lg:col-span-2 bg-[var(--brand-teal)] rounded-xl p-6 flex items-start justify-between gap-6">
            <div className="flex-1">
              ...
              <p className="text-white/80 text-sm leading-relaxed mb-4">
                Revise los eventos adversos con imputabilidad &apos;Definitiva&apos; o &apos;Probable&apos; para identificar posibles señales de seguridad y su reporte regulatorio.
              </p>
