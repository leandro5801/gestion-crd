...
import { mockSitiosClinicos, type SitioClinico } from '@/lib/mock-data'

export function SitiosClinicosContent() {
  const [search, setSearch] = useState('')
  const [estado, setEstado] = useState('Todos los estados')
  const [ubicacion, setUbicacion] = useState('Global')
  const [page, setPage] = useState(1)
  const [showModal, setShowModal] = useState(false)
  const [selected, setSelected] = useState<SitioClinico | null>(null)

  const filtered = mockSitiosClinicos.filter((s) => {
    const matchSearch =
      s.nombre.toLowerCase().includes(search.toLowerCase()) ||
      s.codigo.toLowerCase().includes(search.toLowerCase()) ||
      s.investigadorPrincipal.toLowerCase().includes(search.toLowerCase())
    const matchEstado = estado === 'Todos los estados' || s.estado === estado
    return matchSearch && matchEstado
  })

  const totalSitios = 128
  const sitiosActivos = 112
  const zonas = 24

  return (
    <>
    <NuevoSitioModal open={showModal} onClose={() => setShowModal(false)} />
    <VerEditarSitioModal sitio={selected} onClose={() => setSelected(null)} />
      ...
      <div className="bg-white rounded-xl border border-[var(--border)] p-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {['Todos los estados', 'Activo', 'Pendiente', 'Inactivo'].map((s) => (
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="px-3 py-2.5 border border-[var(--border)] rounded-lg text-sm bg-white focus:outline-none focus:border-[var(--brand-teal)] transition-colors"
            >
              {['Global', 'España', 'Europa', 'LATAM'].map((s) => (
                <option key={s}>{s}</option>
              ))}
            </select>
          </div>
      ...
      <div className="bg-white rounded-xl border border-[var(--border)]">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[var(--border)]">
                {['Nombre del Sitio', 'Código', 'Ubicación', 'Investigador Pral.', 'Estado', 'Estudios', 'Acción'].map((h) => (
              ...
              {filtered.map((s) => (
                <tr key={s.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--muted)] transition-colors">
                  <td className="px-5 py-4">
                    ...
                    <p className="text-[10px] font-semibold uppercase tracking-wide text-[var(--muted-foreground)] mt-0.5">
                      {s.tipo}
                    </p>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4 text-sm text-[var(--foreground)]">{s.ubicacion}</td>
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-[var(--brand-teal)] flex items-center justify-center flex-shrink-0">
                        <span className="text-[10px] font-bold text-white">{s.avatar}</span>
                      </div>
                      <span className="text-sm text-[var(--foreground)]">{s.investigadorPrincipal}</span>
                    </div>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4 text-sm font-bold text-[var(--foreground)]">
                    {String(s.estudiosActivos).padStart(2, '0')}
                  </td>
                  <td className="px-5 py-4">
                    <button
        ...
        <div className="px-5 py-4 flex items-center justify-between border-t border-[var(--border)]">
          <p className="text-sm text-[var(--muted-foreground)]">
            Mostrando <span className="font-semibold text-[var(--foreground)]">{filtered.length}</span> de{' '}
            <span className="font-semibold text-[var(--foreground)]">{totalSitios}</span> sitios clínicos
          </p>
          <DataPagination current={page} total={32} onChange={setPage} />
        </div>
      </div>
