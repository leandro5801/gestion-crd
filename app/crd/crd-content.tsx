...
import { Search, FileDown, FileText, CheckCircle, XCircle, Clock, Eye, Edit, Plus } from 'lucide-react'
...
import { mockCRDs, type CRD } from '@/lib/mock-data'
...
export function CRDContent() {
  ...
  const estudios = ['Todos', ...Array.from(new Set(mockCRDs.map((c) => c.estudio)))]

  const filtered = mockCRDs.filter((c) => {
    const matchSearch =
      c.pacienteId.toLowerCase().includes(search.toLowerCase()) ||
      c.iniciales.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const total = mockCRDs.length
  const enCurso = mockCRDs.filter((c) => c.estado === 'En curso').length
  const completos = mockCRDs.filter((c) => c.estado === 'Completo').length
  const bloqueados = mockCRDs.filter((c) => c.estado === 'Bloqueado').length
  const consentidos = mockCRDs.filter((c) => c.consentimientoFirmado).length

  return (
    <>
    <NuevoCRDModal open={showModal} onClose={() => setShowModal(false)} />
    <VerEditarCRDModal crd={selected} onClose={() => setSelected(null)} />
    <div className="p-6 flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          label="Completados"
          value={completos}
          badge={`${Math.round((completos / total) * 100)}%`}
          badgeVariant="green"
      ...
      <div className="bg-white rounded-xl border border-[var(--border)]">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[var(--border)]">
                {['Paciente (ID/Iniciales)', 'Estudio', 'Fecha Creación', 'Consentimiento', 'EA', 'Adm.', 'Estado', 'Investigador', 'Acciones'].map((h) => (
                ...
                <tr key={c.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--muted)] transition-colors">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-[var(--brand-teal)]">{c.pacienteId}</p>
                        <p className="text-xs text-[var(--muted-foreground)]">{c.iniciales}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4">
                    <span className={`text-sm font-bold ${c.eventosAdversos > 0 ? 'text-red-600' : 'text-[var(--muted-foreground)]'}`}>
                      {c.eventosAdversos}
                    </span>
                  </td>
                  <td className="px-5 py-4 text-sm text-[var(--foreground)]">{c.administraciones}</td>
                  <td className="px-5 py-4">
                    <StatusBadge status={c.estado} />
                  </td>
                  <td className="px-5 py-4 text-xs text-[var(--muted-foreground)]">
                    {c.firma || <span className="italic">Sin firma</span>}
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      <button
        ...
        <div className="px-5 py-4 flex items-center justify-between border-t border-[var(--border)]">
          <p className="text-sm text-[var(--muted-foreground)]">
            ...
            <span className="font-semibold text-[var(--foreground)]">{filtered.length}</span> registros
          </p>
